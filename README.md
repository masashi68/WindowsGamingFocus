...
2. [powershell]と入力し、 [Shift]+[CTRL]を押しながら[Enter]を押します。
3. [このアプリが変更を加えることを許可しますか]で[はい]を選択します。
4. 下記のコマンドをコピーし、Powershellの青い画面に張り付けて[Enter]を押します。（右端の四角いアイコンをクリックするとコピーされます）
```
...
```
5. 以下のメッセージが表示されたら何かキー（スペース等）を押します。

   "PRESS any KEY to Continue!"
   
6. 後は待つだけ。処理が終わると自動的に再起動します。


以下の文章はFork元から変更なしですですが、プログラム本体はゲームの快適性向上に関係ない部分を削っています。
